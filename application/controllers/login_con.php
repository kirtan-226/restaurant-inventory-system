...
class Login_con extends CI_Controller {
	...
	public function login_validation() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$verify_login = $this->accounts_model->login($username,$password);
			if ($verify_login) {
				...
				// if ($hash) {
					...
					$this->session->set_flashdata('successMessage','<div class="alert alert-success">You are now logged in, Welcome '.$this->session->userdata['username'].'</div>');
